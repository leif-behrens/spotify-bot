...
class SecureDashboard:
    ...
    def __init__(self, config_manager: ConfigManager, monitoring_service: SpotifyMonitoringService):
    ...
    def _configure_flask(self) -> None:
        ...
        if not secret_key:
            ...
            logger.warning("Using generated secret key - set FLASK_SECRET_KEY for production")
    ...
    def _register_routes(self) -> None:
        ...
        def dashboard():
            ...
            try:
                ...
                return render_template('error.html', 
                                     error_message="Dashboard konnte nicht geladen werden"), 500
            ...
            try:
                ...
                if status.get('current_track'):
                    safe_status['current_track'] = {
                        ...
                        'artist': self._sanitize_input(status['current_track'].get('artist', '')),
                        'listening_duration': max(0, status['current_track'].get('listening_duration', 0)),
                        'added_to_playlist': status['current_track'].get('added_to_playlist', False)
                    }
        ...
        def api_statistics():
            ...
            try:
                ...
                safe_stats = {
                    ...
                    'tracks_added_to_playlist': max(0, stats.get('tracks_added_to_playlist', 0)),
                    'discovery_rate': round(max(0, stats.get('discovery_rate', 0)), 2),
                    'average_listening_duration_seconds': round(max(0, stats.get('average_listening_duration_seconds', 0)), 2)
                }
                ...
                safe_stats['top_artists'] = [
                    {
                        'artist_name': self._sanitize_input(artist.get('artist_name', '')),
            ...
            try:
                ...
                if hasattr(self.monitoring_service, 'start_from_stored_token'):
                    if self.monitoring_service.start_from_stored_token():
                        return jsonify({'success': True, 'message': 'Service gestartet'})
                
                # Falls das nicht funktioniert, prüfe ob bereits authentifiziert
                if hasattr(self.monitoring_service, 'authenticator') and self.monitoring_service.authenticator.is_authenticated():
                    if not self.monitoring_service.is_running:
                        self.monitoring_service.start()
                        return jsonify({'success': True, 'message': 'Service gestartet'})
                    else:
                        return jsonify({'success': False, 'message': 'Service läuft bereits'})
                else:
                ...
                return jsonify({'success': False, 'message': 'Service konnte nicht gestartet werden'}), 500
        ...
        def start_service_after_auth():
            ...
            try:
                if hasattr(self.monitoring_service, 'start_from_stored_token'):
                    ...
                    if success:
                        return jsonify({'success': True, 'message': 'Service erfolgreich gestartet'})
                    else:
                        return jsonify({'success': False, 'message': 'Service konnte nicht mit gespeicherten Token gestartet werden'})
                else:
                    return jsonify({'success': False, 'message': 'Service-Methode nicht verfügbar'})
                    
            except Exception as e:
                logger.error(f"Failed to start service after auth: {e}")
                return jsonify({'success': False, 'message': f'Service konnte nicht gestartet werden: {str(e)}'}), 500
        ...
        def stop_service():
            ...
            try:
                ...
                return jsonify({'success': False, 'message': 'Service konnte nicht gestoppt werden'}), 500
        ...
        def debug_info():
            ...
            try:
                ...
                if self.monitoring_service:
                    debug_data['service_status'] = self.monitoring_service.get_service_status()
