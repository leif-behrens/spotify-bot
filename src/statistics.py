...
class StatisticsDatabase:
    ...
    def _initialize_database(self) -> None:
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(
                    "CREATE INDEX IF NOT EXISTS idx_current_tracks_track_id ON current_tracks(track_id)"
                )
                cursor.execute(
                    "CREATE INDEX IF NOT EXISTS idx_current_tracks_started_at ON current_tracks(started_at)"
                )
                cursor.execute(
                    "CREATE INDEX IF NOT EXISTS idx_playlist_additions_track_id ON playlist_additions(track_id)"
                )
                cursor.execute(
                    "CREATE INDEX IF NOT EXISTS idx_playlist_additions_added_at ON playlist_additions(added_at)"
                )
                cursor.execute(
    ...
    def update_track_end(
        self, record_id: int, ended_at: datetime, added_to_playlist: bool = False
    ) -> None:
        """
    ...
    def record_playlist_addition(
        ...
        try:
            ...
            with self._get_connection() as conn:
                ...
                record_id = cursor.lastrowid
                logger.info(
                    f"Recorded playlist addition: {track_name} (Duration: {listening_duration}s)"
                )
                return record_id
    ...
    def record_metric(
        self, metric_name: str, metric_value: Any, metadata: Optional[Dict] = None
    ) -> None:
        """
        ...
        try:
            ...
            with self._get_connection() as conn:
                ...
                return {
                    ...
                    "discovery_rate": (added_tracks / total_tracks * 100)
                    if total_tracks > 0
                    else 0,
