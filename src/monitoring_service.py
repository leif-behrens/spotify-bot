...
class SpotifyMonitoringService:
    ...
    def initialize(self) -> None:
        ...
        """
        try:
            logger.info(f"Initializing Spotify Monitoring Service (Session: {self.session_id})")
            ...
            self.playlist_manager = PlaylistManager(self.spotify_client, playlist_config)
    ...
    def initialize_from_stored_token(self) -> bool:
        ...
        """
        try:
            logger.info(f"Initializing Spotify Monitoring Service from stored token (Session: {self.session_id})")
            ...
            logger.info("Service initialization from stored token completed successfully")
            return True
            
        except Exception as e:
            logger.error(f"Service initialization from stored token failed: {e}")
            self.statistics_db.record_metric('service_initialization_from_token_failed', str(e))
            return False
    ...
    def start_from_stored_token(self) -> bool:
        ...
        with self._start_lock:  # Thread-safe Start
            try:
                ...
                logger.error(f"Failed to start monitoring service from stored token: {e}")
                self.statistics_db.record_metric('service_start_from_token_failed', str(e))
                return False
    ...
    def stop(self) -> None:
        ...
        try:
            ...
            self.statistics_db.record_metric('service_stopped', 'success', {'session_id': self.session_id})
    ...
    def _monitor_current_track(self) -> None:
        ...
        try:
            ...
            with self._lock:
                ...
                logger.debug(f"Track data keys: {list(track.keys()) if isinstance(track, dict) else 'not a dict'}")
                ...
                # Neuer Track?
                if not self.current_session or self.current_session.track_id != track_id:
                    # Beende alte Session
                    if self.current_session:
                        self._end_current_session()
    ...
    def _start_new_session(self, track: Dict[str, Any], playback_data: Dict[str, Any]) -> None:
        """
        ...
        try:
            ...
            record_id = self.statistics_db.record_current_track(track_data, self.session_id)
            self.current_session.database_record_id = record_id
            
            logger.debug(f"Started new track session: {track_data['track_name']} by {track_data['artist_name']}")
    ...
    def _end_current_session(self) -> None:
        ...
        try:
            ...
            logger.debug(f"Ended track session: {self.current_session.track_data['track_name']} "
        ...
        try:
            ...
            if current_duration >= min_duration:
                ...
                if success:
                    ...
                    logger.info(f"Added track to playlist after {current_duration}s: "
