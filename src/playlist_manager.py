...
class PlaylistManager:
    ...
    def __init__(self, spotify_client: spotipy.Spotify, playlist_config: Dict[str, str]):
    ...
    def _validate_track_data(self, track_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if 'track_id' in track_data:
                ...
                logger.warning(f"Track missing ID field. Available fields: {list(track_data.keys())}")
                return False
            ...
            if 'artists' in track_data:
                if not isinstance(track_data['artists'], list) or len(track_data['artists']) == 0:
                    logger.warning("Track has no valid artists")
                    return False
            elif 'artist_name' in track_data:
                if not isinstance(track_data['artist_name'], str) or len(track_data['artist_name'].strip()) == 0:
    ...
    def initialize_playlist(self) -> str:
        ...
        try:
            ...
            logger.info(f"Created new playlist: {self.playlist_name} (ID: {self.auto_playlist_id})")
    ...
    def _find_existing_playlist(self, user_id: str) -> Optional[str]:
        ...
        try:
            ...
            while True:
                self._rate_limit_sleep()
                playlists = self.spotify.user_playlists(user_id, limit=limit, offset=offset)
    ...
    def add_track_to_playlist(self, track_uri: str, track_info: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if self._is_track_in_playlist(track_uri):
                ...
                track_name = track_info.get('track_name') or track_info.get('name', 'Unknown')
