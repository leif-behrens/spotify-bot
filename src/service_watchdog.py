...
class ServiceWatchdog:
    ...
    def __init__(self, 
        ...
        logger.info(f"ServiceWatchdog initialized (check_interval={check_interval_seconds}s, max_failures={max_failures})")
    ...
    def _perform_health_check(self) -> None:
        ...
        try:
            with self._health_lock:
                ...
                self.health_status.uptime_seconds = int((current_time - self.start_time).total_seconds())
                ...
                if is_healthy:
                    ...
                    logger.warning(f"Service health check failed (attempt {self.health_status.consecutive_failures}/{self.max_failures})")
    ...
    def _check_service_health(self) -> bool:
        ...
        try:
            ...
            if self.service.error_count > max_allowed_errors:
                logger.debug(f"Health check failed: Too many errors ({self.service.error_count})")
                return False
    ...
    def _attempt_service_restart(self) -> None:
        ...
        try:
            ...
            logger.warning(f"Attempting service restart (attempt #{self.restart_count + 1})")
            ...
            backoff_delay = min(self.restart_delay * (2 ** min(self.restart_count, 5)), 300)  # Max 5min
            ...
            if self.service.start_from_stored_token():
                ...
                logger.info(f"Service successfully restarted (downtime: {downtime_duration}s)")
                ...
                self.health_status.last_error = "Restart failed: Authentication required"
        ...
        with self._health_lock:
            ...
            return {
                ...
                'availability_percent': round(
                    ((uptime_seconds - self.total_downtime_seconds) / max(uptime_seconds, 1)) * 100, 2
                ),
